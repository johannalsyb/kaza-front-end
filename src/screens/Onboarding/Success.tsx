import { ImageBackground, Pressable, ScrollView, StyleSheet, useWindowDimensions, View, Platform } from "react-native"
...
const mimg = "../../assets/Onboarding/Congrats.webp"
const dimg = "../../assets/Onboarding/Desktop_Congrats.jpg"
                    ...
                    !isMobile && (
                        ...
                        >
                            <View style={[styles.containerText, isMobile && { marginTop: 0 }]} >
                                <KText
                                    style={{
                                        fontWeight: isMobile ? '700' : '600', fontSize: calculateFontSize(),
                    ...
                    <KButton
                        ...
                        onPress={async () => {
                            try {
                                ...
                                // Save onboarding completion timestamp
                                await storage.setItem("onboardingFinishedAt", new Date().toISOString());

                                // Update user onboarding status
                                await users.me.update({
                                    onboarding: JSON.stringify({ step: 5, data: {}, completed: true }),
                                });

                                // ✅ Navigation
                                if (Platform.OS === "web") {
                                    document.location.href = "/"; // Force reload on web
                                } else {
                                    props.navigation.navigate("Home"); // Use RN navigation on native
                                }
                            } catch (e) {

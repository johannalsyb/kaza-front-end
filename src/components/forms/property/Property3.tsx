...
import KImageUpload, {
  Handle as KImageUploadHandle,
  KImageUploadWidthDesktop,
  KImageUploadWidthMobile,
} from '../../KImageUpload';
...
type Props = {
  ...
  property: Property;
  showRotate?: boolean;
  showImageNotification?: boolean;
  setShowImageNotification?: (val: boolean) => void;
};
...
export default (props: Props) => {
  ...
  const [primaryImage, setPrimaryImage] = useState<string | undefined>(
    props.property.primaryImage ||
      (props.property.pics ? props.property.pics[0] : undefined),
  );
  const {isMobile} = useIsMobile();
  const iuRef = useRef<KImageUploadHandle>(null);
  const [imageLoading, setImageLoading] = useState<(false | string)[]>([]);
  const [picsRotate, setPicsRotate] = useState<number[]>(
    new Array(pics.length).fill(0),
  );
  const [imagesLoading, setImagesLoading] = useState<boolean[]>(
    new Array(pics.length).fill(false),
  );
  ...
  const uploadFn = async (b64array: string[]) => {
    ...
    const imgs = b64array.map(b64 =>
      b64.startsWith('data:image/') ? b64.split(',')[1] : b64,
    );
  ...
  const onLoadedFn = (b64array: (string | null)[]) => {
    b64array.forEach(b64 => {
      if (b64)
        pics.push(b64.startsWith('data:image/') ? b64.split(',')[1] : b64);
    });
    // console.log(pics)
    setPics([...pics]);
    setImageLoading(new Array(pics.length).fill(false));
  ...
  const rotateFn = async (id: string, degrees: number) => {
    if (!props.property.id) return;
    const res = await properties.pictures.rotate(
      props.property.id,
      id,
      degrees,
    );
    ...
    <View
      ...
      <FormField
        ...
        <KDragAndDropZone
          ...
          onDrop={f => {
            Promise.all(f).then(ff => iuRef.current?.onFiles(ff));
          }}
        />
        {/* pics notification */}

        {props?.showImageNotification && (
          <View
            style={[
        ...
        <View
          style={{
            maxWidth:
              (isMobile ? KImageUploadWidthMobile : KImageUploadWidthDesktop) *
                3 +
              2 * 20,
            display: 'flex',
            flexDirection: 'row',
            ...
            <KImageUpload
              ...
              onLoading={loading => {
                if (loading) {
                  ...
                  setImageLoading([
                    ...imageLoading.slice(0, imageLoading.length - 1),
                    false,
                  ]);
                }
              }}
          ...
          {pics.length < nbSquares
            ? new Array(pics.length > nbSquares ? 1 : nbSquares - pics.length)
                .fill(0)
                .map((_, i) => {
                  return (
                    <Pressable
                      ...
                      {imageLoading[i + pics.length] ? (
                        <View
                          ...
                          <Image
                            source={{
                              uri: imageLoading[i + pics.length] as string,
                            }}
                            style={{
                              width: '100%',
                              height: '100%',
